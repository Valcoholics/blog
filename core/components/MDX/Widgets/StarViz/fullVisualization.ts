...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // Simulate bass (no audio in sandpack)
  ...
  // Simplified blur (3x3 instead of 5x5 for performance)
  vec3 blurred = color;
    ...
    for (int j = -1; j <= 1; j++) {
      if (i != 0 || j != 0) {
        ...
        if (sampleUV.x >= 0.0 && sampleUV.x <= aspect &&
          ...
          vec3 sampleCombined = vec3(sampleCircle1) * 0.7 +
...
export default FullVisualization;
